( function( $, wp ) {
	...
	customInlineEditPost.edit = function(id) {
		...
		if ( $( ':input[name="popular"]', editRow ).length === 1 ) {
			if (!isNaN(parseInt($('#post-' + id + ' .ws365150_popular').text()))) {
				$( ':input[name="popular"]', editRow ).val(parseInt($('#post-' + id + ' .ws365150_popular').text()));
			}
		}
		...
		$('input[name="popular"] + #clear_value').on('click', function () {
			$( ':input[name="popular"]', editRow ).val('');
			return false;
		});
