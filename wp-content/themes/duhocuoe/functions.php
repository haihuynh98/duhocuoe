...
if (!function_exists('theme_setup')) {
    ...
    function theme_setup()
    {
        ...
        $GLOBALS['npThemeVersion'] = '4.12.14';
    }
...
function theme_scripts()
{
    ...
    wp_localize_script('theme-script', 'screenReaderText', array(
        'expand' => __('expand child menu', 'annammep'),
        'collapse' => __('collapse child menu', 'annammep'),
    ));
}
...
function theme_widgets_init2()
{
    register_sidebar(array(
        'name' => __('Primary Widget Area', 'annammep'),
        'id' => 'primary',
        'description' => __('Add widgets here to appear in your sidebar on blog posts and archive pages.', 'annammep'),
...
function _theme_get_excerpt($args = array())
{
    ...
    $more_tag = isset($args['more_tag']) ? $args['more_tag'] : __('Read more', 'annammep');
...
function theme_get_category_list()
{
    return str_replace(
        '<a',
        '<a class="u-textlink"',
        get_the_category_list(_x(', ', 'Used between list items, there is a space after the comma.', 'annammep'))
    );
}

function theme_get_tags_list()
{
    return get_the_tag_list('', _x(', ', 'Used between list items, there is a space after the comma.', 'annammep'));
}
...
function getProductButtonHtml($button_html, $product, $clickTypeProductbutton = 'add-to-cart', $contentProductbutton = '')
{
    ...
    $buttonText = sprintf(__('Add to cart', 'annammep'));
    if ($contentProductbutton !== '') {
        $buttonText = sprintf(__($contentProductbutton, 'annammep'));
    }
...
{
    $args = wp_parse_args($args, array(
        'error_title' => __('Nothing here', 'annammep'),
        'error_message' => __('It looks like nothing was found at this location. Maybe try a search?', 'annammep')
    ));
    extract($args);
...
{
    wp_enqueue_style('boostrap-style', get_template_directory_uri() . '/boostrap/bootstrap.min.css', [], false);
    ...
    wp_enqueue_style('font-style', get_template_directory_uri() . '/fonts/font-style.css', [], false);
    ...
    wp_enqueue_style('footer', get_template_directory_uri() . '/css/footer.min.css', [], false);
    if (is_singular('production')) {
        wp_enqueue_style('production', get_template_directory_uri() . '/css/production-dist.css', [], false);
    }
    ...
    if (is_home()) {
        ...
        wp_enqueue_style('section-member', get_template_directory_uri() . '/css/section-member.css');
        wp_enqueue_script('swiperjs', get_template_directory_uri() . '/swiper/swiper-bundle.min.js', null, null, true);
        wp_enqueue_script('swiper-home', get_template_directory_uri() . '/js/swiper-home.js', null, null, true);
        wp_enqueue_script('swiper-client', get_template_directory_uri() . '/js/swiper-client.js', null, null, true);
